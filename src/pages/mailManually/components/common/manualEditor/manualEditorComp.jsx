...
const MailEditor = () => {
  ...
  const [isFeedbackFail, setIsFeedbackFail] = useState(false); 
  ...
  const handleSave = async () => {
    try {
      ...
      console.error('메일 저장 실패', error.message);
      setDraftFailed(true);
    } finally {
      setIsMailSaved(true);
    }
    ...
    <>
            ...
            <ContentBox>
              ...
              <Note>
                * <span style={{ color: 'red' }}>빨간색 글씨</span>는 변경된
                부분, <del style={{ color: 'gray' }}>회색 취소선</del>은 삭제된
                부분을 의미합니다.
      ...
      {isSendFail && (
        <AlertModal
          title="메일 전송에 실패했습니다."
          buttonText="확인"
          onButtonClick={() => setIsSendFail(false)}
        />
      )}
      ...
      {isFeedbackFail && (
        <AlertModal
          ...
          buttonText="확인"
          onButtonClick={() => setIsFeedbackFail(false)}
        />
      )}
    </>
  );
